...
export default function Skills() {
    ...
    const iconMap = {
        "Lenguajes & Frameworks": <Terminal className="text-lila-500" size={24} />,
        "Bases de Datos": <Database className="text-lila-500" size={24} />,
        "Herramientas & Otros": <Settings className="text-lila-500" size={24} />
    };
    ...
    return (
        <section id="skills" className="py-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {skills.map((cat, i) => (
                    <motion.div
                        ...
                        className="p-6 bg-slate-800/10 rounded-2xl border border-slate-700/50 hover:border-lila-500/30 transition-all duration-300"
                    >
                        <div className="flex items-center gap-3 mb-6">
                            {/* Ponemos el icono según el nombre de la categoría */}
                            {iconMap[cat.category] || <Terminal className="text-lila-500" size={24} />}
                        ...
                        <ul className="space-y-3">
                            {cat.items.map((skill, idx) => (
                                <li key={idx} className="flex items-center gap-2 text-slate-400 font-mono text-xs">
                                    <span className="text-lila-500/50">❯</span>
                                    {skill}
                                </li>
                            ))}
                        </ul>
            ...
            <motion.div
                ...
                className="mt-12 p-6 border-l-2 border-lila-500/30 bg-lila-500/5 rounded-r-xl"
            >
                <p className="text-slate-400 text-sm italic leading-relaxed">
                    <ShieldCheck size={16} className="inline mr-2 text-lila-500" />
